...
def init_params(options, inputFile, targetFile):
    ...
    if os.path.exists(options['save_to']):
        load_params(options['save_to'], params)
...
def predict_process(options):
    ...
    if options['input_sen']!='':
        if len(options['input_sen'].split())<int(options['7or5']):
            ll = options['input_sen'].split()
            for i in range(int(options['7or5'])-len(ll)):
                options['input_sen']+=' '
                options["input_sen"]+=ll[-1]
        test = [options['input_sen']]
        target = [options['input_sen']]
        print options['input_sen']
    ...
    for t in range(lentest):
        ...
        if options['top_k']==0:
            ...
            if len(startWords):
                ...
                if options['not_first']==0:
                    for i in range(len(options['predict_seq_len'])):
                        ran.append(int(options['7or5']))
                else:
                    for i in range(len(options['predict_seq_len'])-1):
                        ran.append(int(options['7or5']))
...
if __name__ == '__main__':
    ...
    if not os.path.exists('config.txt'):
        ...
        if options['poem_type'].find('poem')!=-1:
            options['emb_dim'] = 150
        ...
        if options['poem_type'].find('poem')!=-1:
            options['7or5'] = options['poem_type'].replace('poem', '')
            options['poem_type'] = options['poem_type'].replace(options['7or5'], '')

        options['yunmu_file'] = options['path']+options['poem_type']+'/'+'yun_utf8.txt'
        options['test_head_file'] = options['path']+options['poem_type']+'/'+'head_cover_poem_5k.txt'
        options['dui_lian'] = options['path']+options['poem_type']+'/'+'liewngduiyun_dui.txt'
        options['word_l_file'] = options['path']+options['poem_type']+'/'+'yunmu.txt'



        if options['poem_type'].find('poem')!=-1:
            options['wordsFilePath'] = options['path']+options['poem_type']+'/'+'word2vec_cover_poem_5k.txt'
            options['inputFilePath'] = options['path']+options['poem_type']+'/'+'cover_poem_5k.txt'
            options['targetFilePath'] = options['path']+options['poem_type']+'/'+'cover_poem_5k.txt'
            options['save_to'] = options['path']+options['poem_type']+'/'+'model_cover_poem_5k.npz'
            options['test_in_file'] = options['path']+options['poem_type']+'/'+'test_cover_poem_5k.txt'
            options['test_out_file'] = options['path']+options['poem_type']+'/'+'test_cover_poem_5k.txt'

            # options['predict_file'] = options['path']+options['poem_type']+'/'+'out_cover_poem_5k_top5_7_theme.txt'

            options['googleVec'] = options['path']+options['poem_type']+'/'+'si_vectors.txt'

            lv_l0 = []
            lv_l1 = []
            for l in options['lv_7'].replace(' ', '').replace('\n', '').split('/'):
                temp_l = []
                for ll in l.split(','):
                    temp_l.append(ll)
                lv_l0.append(temp_l)
            for l in options['lv_5'].replace(' ', '').replace('\n', '').split('/'):
                temp_l = []
                for ll in l.split(','):
                    temp_l.append(ll)
                lv_l1.append(temp_l)
            ...
            options['head'] = options['head'].replace(' ', '').replace('/', ' ')

            options['input_sen'] = options['input_sen'].replace(' ', '').replace('/', ' ')

            if len(options['head'].decode('utf-8'))>5:
                options['cut_to_one'] = 1
            else:
                options['cut_to_one'] = 0
